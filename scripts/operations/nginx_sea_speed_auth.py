...
def verify_source_layout(text: str, host: str = "mostdef.ru") -> None:
    ...
    for spec, _start, _open_index, _close_index in _location_blocks(server_text):
        ...
        if uri == SEA_SPEED_PREFIX or uri.startswith(SEA_SPEED_PREFIX + "/"):
            return
    if any(_is_sea_speed_include_path(match.group("path")) for match in INCLUDE_DIRECTIVE_RE.finditer(server_text)):
        return
...
def _api_origin(server_text: str) -> str:
    ...
    for spec, _start, open_index, close_index in _location_blocks(server_text):
        ...
        if parsed.hostname not in {"127.0.0.1", "localhost", "::1"}:
            raise ConfigError("Sea Speed API upstream must remain loopback for private worker ingress")
        if parsed.username or parsed.password or not parsed.port:
            raise ConfigError("Sea Speed API upstream must be credential-free loopback host:port")
...
def _worker_private_block(listen: str, peer: str, api_origin: str) -> str:
    ...
    if len({path for path, _method in endpoints}) != len(endpoints):
        raise ConfigError("private worker ingress endpoints must be unique")
...
def render(
    ...
    sea_locations = [
        ...
        if _location_uri(spec) == SEA_SPEED_PREFIX or _location_uri(spec).startswith(SEA_SPEED_PREFIX + "/")
    ]
...
def verify(
        ...
        if uri == SEA_SPEED_PREFIX or uri.startswith(SEA_SPEED_PREFIX + "/"):
            ...
            for marker in required:
                if marker not in body:
                    raise ConfigError(f"unprotected Sea Speed location {spec}: missing {marker}")
    ...
    if worker_private_listen or worker_private_peer:
        ...
        if expected not in text:
            raise ConfigError("private worker ingress block missing or does not match expected addresses")
...
def main() -> int:
    ...
    if args.command == "materialize":
        ...
        materialized = materialize_sea_speed_includes(text, args.host, include_root=args.include_root)
