#!/usr/bin/env python3
"""Build a non-secret Sea Speed release provenance manifest from an allow policy decision."""
...
from production_policy import validate_decision
...
def load_policy_decision(path: Path, source_commit: str) -> dict[str, object]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    validate_decision(payload, require_allow=True)
    if payload.get("sourceCommit") != source_commit:
        raise ValueError("policy decision sourceCommit mismatch")
    approved = payload.get("approvedFiles")
    if not isinstance(approved, list) or not approved or approved != sorted(set(approved)):
        raise ValueError("policy decision approvedFiles must be a sorted non-empty unique list")
    for key in ("outcomeContractHash", "changeContractHash", "policyHash", "decisionId"):
        value = payload.get(key)
        if not isinstance(value, str) or not DIGEST_RE.fullmatch(value):
            raise ValueError(f"policy decision {key} must be SHA-256")
    return payload
...
def build_manifest(args: argparse.Namespace) -> dict[str, object]:
    ...
    decision = load_policy_decision(args.policy_decision_evidence, source_commit)
    approved_files = list(decision["approvedFiles"])
    ...
    binding = {
        ...
        "outcomeContractHash": decision["outcomeContractHash"],
        "changeContractHash": decision["changeContractHash"],
        "approvedFiles": approved_files,
    }
    approved_scope_hash = scope_hash(binding)
    evidence: dict[str, str] = {
        "policyDecisionSha256": manifest_digest(args.policy_decision_evidence),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--policy-decision-evidence", required=True, type=Path)
