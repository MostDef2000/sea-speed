...
- TASK-008: If TASK-007 exposes a real deploy-script defect requiring any path outside this 17-path scope, stop writes and obtain a fresh Implementation Scope Check plus `OUTCOME APPROVED`; do not alter `deploy/vps/deploy.sh` under this authorization.
...
- AC-008 | Task: TASK-001,TASK-009,TASK-010 | Evidence: exact 17-path Git compare, CONTROL_PLANE Change Contract and exact-head/post-merge CI | Coverage: COVERED

## Definition of Done

- [x] Issue/spec/plan/tasks current: Issue #184 and feature 016 describe the accepted process outcome and exact scope.
- [ ] Exact changed-file scope verified: final branch diff must equal the approved 17 paths with no runtime source expansion.
- [ ] Required tests and evidence complete: parser, workflow policy/architecture, transaction harness and repository behavioral tests must pass on the final exact head.
- [ ] Required CI green: PR Validation and aggregate Quality integration must both succeed on the same final head.
