...
After authorization, the Delivery Orchestrator must continue through every safe deterministic action it can perform. PR creation, a remediable source/test/metadata/CI failure, merge readiness, packaging, deployment preparation or deployment start are progress events and cannot end the turn. A known GitHub Actions run or check that is `queued` or `in_progress` is also a progress event: the Orchestrator remains `ACTIVE`, foreground-waits, and re-observes only the exact run/check cursor through the official Connector at least 30 seconds after the previous equivalent status observation, with no observation count or deadline that itself hands control back, no busy/tight polling, and no checkpoint-generation churn per observation; success continues automatically to the next gate and failure immediately narrows to failed job/log remediation. The Orchestrator returns control only as `DONE` when the Outcome is actually complete, `BLOCKED` when a concrete external blocker prevents continuation and its evidence/unblock condition/next action are known, or `HUMAN DECISION REQUIRED` when a genuine protected human choice is needed and the exact options/consequences/reply format are stated. `FAILED` is an event to remediate or classify, never a terminal interaction state.
...
- FR-017: The Delivery Orchestrator MUST return control only as `DONE`, `BLOCKED`, or `HUMAN DECISION REQUIRED`. `BLOCKED` MUST identify a concrete external blocker, evidence, unblock condition and next admissible action; `HUMAN DECISION REQUIRED` MUST identify the exact protected decision, bounded alternatives/consequences where applicable and exact reply/action format; `FAILED` MUST be treated as a remediable/classifiable event rather than a terminal interaction state. Deterministic authorized work MUST continue automatically until one of these states is justified. A known GitHub Actions run or check that is `queued` or `in_progress` MUST keep the invocation `ACTIVE` with foreground rate-limited exact-cursor observation and MUST NOT be represented as `WAITING_EXTERNAL`.
...
- AC-018: The same six contracts explicitly state that a known GitHub Actions run or check that is `queued` or `in_progress` keeps the Orchestrator `ACTIVE` with foreground rate-limited exact-cursor observation, does not justify `WAITING_EXTERNAL`, and distinguishes a Connector/provider capability outage as `BLOCKED` or `HUMAN DECISION REQUIRED`.
...
- NFR-009 | Area: Orchestration reliability | Target: Every Delivery Orchestrator turn with active work continues deterministic authorized actions until exactly `DONE`, `BLOCKED`, or `HUMAN DECISION REQUIRED`; no status-only terminal handoff or terminal `FAILED` is permitted; a known `queued`/`in_progress` GitHub Actions run/check remains `ACTIVE` with foreground rate-limited exact-cursor observation and is never represented as `WAITING_EXTERNAL` | Validation: six-contract consistency plus regression test | Evidence: `tests/test_delivery_terminal_states.py`, `AGENTS.md`, `contracts/SEA_SPEED_GOVERNANCE.md`, `contracts/SEA_SPEED_DELIVERY_POLICY.md`, `contracts/runtime/SEA_SPEED_TASK_RUNTIME.md`, `contracts/runtime/RELEASE_READINESS_GATE.md`, `contracts/branches/project-manager.md` | Status: PASS
