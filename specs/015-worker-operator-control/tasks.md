...
The current correct-course source gate is exactly these 5 paths; the historical 17-path PR #179 and 9-path PR #180 scopes remain immutable audit history:
...
Current diff-derived runtime applicability is VPS only. The parent Outcome still has a separately pending Ubuntu worker-control production installation/acceptance from merged PR #180; this 5-path remediation does not rewrite that source history or imply Ubuntu deployment.
...
- AC-015 | Task: T-013, T-014, T-008 | Evidence: tests/test_vps_deploy_origin_health.py plus later runtime 8010 source/health proof | Coverage: COVERED
...
- EVID-009 | Type: integration | Priority: P0 | Covers: AC-015 | Source: tests/test_vps_deploy_origin_health.py | Status: PENDING CURRENT REMEDIATION CI
...
- Issue/spec/plan/tasks current: YES for the active 5-path correct-course remediation; production feedback records run #25 false-negative and the restored healthy VPS baseline.
- Exact changed-file scope verified: must remain exactly the approved 5 remediation paths above.
- Required tests and evidence complete: new VPS origin-health regression test plus full aggregate CI required before remediation merge; AC-012 and runtime part of AC-015 remain runtime-manual after production authorization.
- Required CI green: exact final remediation PR head must have PR Validation and Quality integration SUCCESS.
- Risk-profile applicability: current 5-path PR derives VPS-only impact with Security impact NONE, schema impact NONE, destructive migration NO and other high-risk trigger NO, therefore Change Contract declares `Risk profile: NOT REQUIRED`; the parent feature's existing full risk register remains in plan.md for final mixed runtime acceptance.
...
- Risks resolved or explicitly accepted: RISK-008 source mitigation must pass; parent feature residual runtime risks remain subject to production acceptance.
...
- Active source gate: exact 5-path remediation scope, current SDD, diff-derived `Risk profile: NOT REQUIRED`, non-FAIL quality verdict, new origin-health regression test, PR Validation, Quality integration, review-thread check and expected-head merge.
