...
- T-016 [P0] Replace Deploy VPS first-parent membership pipeline with a full-consumption explicit match flag that remains strict under `set -o pipefail` and rejects non-first-parent targets.
- T-017 [P0] Add regression evidence that forbids the unsafe `git rev-list --first-parent ... | grep -q` pattern, requires the pipefail-safe guard, and record run #26 as a pre-SSH production-learning false-negative.

## Active remediation scope

The current correct-course source gate is exactly these 5 paths; historical 17-path PR #179, 9-path PR #180, 7-path PR #181 and superseded intermediate scopes remain immutable audit history:

1. `.github/workflows/deploy-vps.yml`
2. `tests/quality/test_quality_architecture.py`
...
Current diff-derived production impact is `CONTROL_PLANE`; no VPS, Ubuntu Worker/relay or Windows Worker runtime source is changed. The parent Outcome still has pending VPS runtime acceptance for already-authorized source `1d7c8478a467f28f4519111bae06f5d2f7fa5e61` and pending Ubuntu worker-control acceptance. This control-plane remediation does not create a substitute runtime release identity.
...
- AC-016 | Task: T-016, T-017, T-008 | Evidence: tests/quality/test_quality_architecture.py plus later workflow_dispatch first-parent admission evidence | Coverage: COVERED
...
- EVID-006 | Type: end-to-end | Priority: P0 | Covers: AC-011, AC-014, AC-015, AC-016 | Source: exact-head GitHub Actions | Status: PENDING CURRENT CONTROL-PLANE CI
...
- EVID-009 | Type: integration | Priority: P0 | Covers: AC-015 | Source: tests/test_vps_deploy_origin_health.py and tests/test_camera_preview_gallery.py | Status: GREEN on merged PR #181; aggregate CI will revalidate
- EVID-010 | Type: runtime-manual | Priority: P0 | Covers: AC-015 | Source: next Deploy VPS retry plus `127.0.0.1:8010/api/health` exact source identity / rollback evidence | Status: PENDING
...
- EVID-012 | Type: integration | Priority: P0 | Covers: AC-016 | Source: tests/quality/test_quality_architecture.py | Status: PENDING CURRENT CONTROL-PLANE CI

## Definition of Done

- Issue/spec/plan/tasks current: YES for the active 5-path control-plane correct-course remediation; production feedback records run #26 as a pre-SSH `pipefail` false-negative.
- Exact changed-file scope verified: must remain exactly the approved 5 remediation paths above.
- Required tests and evidence complete: pipefail-safe first-parent regression plus full aggregate CI are required before remediation merge; AC-012 and runtime portions of AC-015/AC-016 remain runtime-manual after control-plane merge.
- Required CI green: exact final remediation PR head must have PR Validation and Quality integration SUCCESS.
- Risk-profile applicability: `Risk profile: REQUIRED` because the PR explicitly declares an other high-risk trigger for production admission-guard logic; plan risk rows must remain complete and mitigated/accepted according to evidence.
- Exact-green-head merge complete: only after fresh main/base/head/scope/review verification and expected-head merge.
- Deployment state resolved: source remediation does not deploy. Current VPS runtime remains restored/healthy at `6bf909c13d48df1d44b87a62d0686b61d8c3af45` on 8010; run #26 stopped before SSH; Ubuntu rollout has not started.
- Release provenance resolved: no runtime release is created by the control-plane merge. Existing runtime source `1d7c8478a467f28f4519111bae06f5d2f7fa5e61` retains its exact quality/provenance/production authorization and remains the intended VPS target after the workflow fix.
...
- Deferred work recorded: merge control-plane fix; retry Deploy VPS for the already-authorized runtime SHA; then authorization/compatibility re-verification for pending Ubuntu installation and final AC-012 acceptance.
- Risks resolved or explicitly accepted: first-parent admission semantics must remain strict and the SIGPIPE false-negative must be regression-protected before merge; final hosted-runner proof occurs on the next dispatch.
- Waivers resolved or current: no waiver is currently required; any future waiver must satisfy the durable waiver contract and cannot bypass first-parent admission, exact artifacts or production authorization.

## Completion gate

- Active source gate: exact 5-path remediation scope, current SDD, `Risk profile: REQUIRED`, non-FAIL quality verdict, pipefail-safe first-parent regression, PR Validation, Quality integration, review-thread check and expected-head merge.
- Current control-plane production gate: VPS/Ubuntu/Windows deployment fields are all `NOT REQUIRED` and `Production safety envelope: NOT REQUIRED` because the exact diff has no runtime contour. The control-plane merge itself must never be used as a substitute runtime release identity.
- Runtime retry gate: after control-plane merge, manually dispatch Deploy VPS from current `main` targeting `1d7c8478a467f28f4519111bae06f5d2f7fa5e61` with Issue #178. Existing runtime authorization fingerprint must still verify before SSH; if any authorization-bound field has changed, stop for a fresh runtime envelope.
- Parent Ubuntu gate: this control-plane PR does not authorize or require an Ubuntu update. Before Ubuntu mutation, re-verify the eligible exact Ubuntu source, applicable production authorization, release artifact/runtime identity and rollout compatibility.
