...
### Scenario 3 - fail closed when control is unavailable

Given the VPS cannot reach or authenticate to the Ubuntu control agent, when the UI refreshes or an operator attempts a worker action, then no alternate command path is used, no arbitrary service can be selected, and the UI reports control unavailability while leaving the live stream untouched.
...
- FR-011: Detection, tracking, speed, calibration and event semantics MUST remain unchanged; no `worker/**` source is modified.
...
- NFR-002 | Area: RELIABILITY | Target: worker-control outage fails closed within bounded timeout and never interrupts HLS | Validation: API proxy error tests plus frontend contract | Evidence: tests/test_worker_operator_control.py and tests/test_frontend_contract.py | Status: PASS
...
- New additive browser API: `/sea-speed/api/worker/control`, `/start`, `/stop`.
- New private Ubuntu agent: fixed status/start/stop HTTP surface on a configured RFC1918 listener.
...
- Runtime acceptance: PENDING separate exact-SHA production authorization.
