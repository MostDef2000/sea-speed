...
Given the dedicated Worker deploy key is presented with a shell, PTY, forwarding request or command other than the exact protocol, when sshd admits the dedicated principal through the existing Worker `AllowUsers` policy and then applies the authorized-key restrictions and forced command, the request cannot obtain general execution and the gate rejects unsupported `SSH_ORIGINAL_COMMAND`. Existing operator principals remain admitted and are not replaced by bootstrap.
...
- FR-007: The Worker deployment account MUST be `sea-speed-deploy`, MUST reject password authentication without being account-locked, MUST be admitted by the existing effective Worker `AllowUsers` boundary when such a boundary is configured, and MUST admit only one authorized Ed25519 key using OpenSSH `restrict` plus forced command. On Linux the active account password field MUST NOT begin with `!`, because sshd rejects a locked account before public-key authentication.
...
- FR-017: One-time bootstrap MUST not create or print a private key. It accepts one Ed25519 public key, may require an independently verified fingerprint, installs the root-owned gate, restricted authorized key and one exact gate sudo rule, validates sudoers, configures the deploy account with a non-locking invalid password marker, and if the effective Worker SSH policy uses the canonical `/etc/ssh/sshd_config.d/00-sea-speed-hardening.conf` `AllowUsers` line, MUST preserve all existing principals while adding only `sea-speed-deploy`, validate with `sshd -t`, reload sshd, verify effective membership, and restore the original file on failure.
- FR-018: Bootstrap MUST support bounded transport-boundary removal without modifying application runtime state; removal MUST delete the deploy key/sudo boundary, remove only `sea-speed-deploy` from the canonical `AllowUsers` list when present, validate/reload sshd, preserve other principals, and lock the dedicated deploy account.
...
- AC-007: Bootstrap tests prove `restrict` + forced command, no `NOPASSWD: ALL`, root-owned gate, password authentication disabled without an active account lock, transactionally preserved/extended canonical `AllowUsers` membership, rollback of invalid sshd config, and no private-key generation/printing.
...
- AC-012: A restricted transport probe authenticates by the dedicated key after effective sshd policy admits `sea-speed-deploy`, while existing operator access remains admitted and shell/PTY/forwarding/non-protocol requests cannot produce general execution.
...
- NFR-002 | Area: LEAST_PRIVILEGE | Target: Worker key can invoke only one root-owned validated gate; password login is impossible; existing SSH allowlist principals are preserved and only the dedicated deploy principal is added; VPS remains jump-only; no broad sudo or self-hosted runner | Validation: static tests + installed authorized_keys/sudoers/account/AllowUsers evidence | Evidence: bootstrap output and read-only installed-boundary inspection | Status: CONCERNS
- NFR-003 | Area: FAIL_CLOSED | Target: private/unprotected source, missing checks, missing transport credentials, malformed request, non-main SHA, artifact mismatch, unknown AllowUsers policy source or invalid sshd config produces no application mutation | Validation: unit/static tests and protected workflow evidence | Evidence: tests + failed-negative workflow/probe evidence | Status: PASS
...
- Root cause #1 was the bootstrap's `passwd -l sea-speed-deploy`: OpenSSH checks account accessibility independently of authentication type, and on Linux a leading `!` lock prevents public-key login too. The remediation switched the active account to the non-locking invalid password marker `*NP*`.
- A second CONNECTOR retry still failed before public-key authorization. Runtime diagnostics proved account state, key fingerprint, ownership/modes, `PubkeyAuthentication yes` and `PasswordAuthentication no` were correct, while effective sshd policy was `AllowUsers seaspeedadmin`; journal evidence stated `User sea-speed-deploy ... not allowed because not listed in AllowUsers`.
- Correct-course remediation now treats canonical Worker `AllowUsers` membership as part of the bounded transport bootstrap: preserve `seaspeedadmin` and any other existing principals, add only `sea-speed-deploy`, validate/reload/verify sshd transactionally, and remove only that principal on transport removal.
