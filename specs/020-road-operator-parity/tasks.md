...
- T-014 [P0] Correct the production-learning delivery capability/action budget in exactly `spec.md`, `plan.md` and `tasks.md`: VPS is `ONE_COMMAND_FALLBACK`, Ubuntu is `ONE_COMMAND_FALLBACK`, operator actions expected `2`; preserve the unchanged authorized runtime release and fingerprint inputs. The corrective PR itself derives `Production impact: NONE` because its diff contains only `specs/**` documentation/SDD paths and creates no executable control-plane or runtime artifact. PENDING corrective PR validation/merge.
