...
- T-006 [x] Remove active three-line/legacy comment request workflows and parsers; retain only the canonical-inventory verifier/policy paths as explicit fail-closed compatibility tombstones with no authority behavior.
...
- T-008 [x] Add/replace policy, workflow, release and architecture tests for self-authorization resistance, source-derived contours, stale-run suppression, fail-closed delegation/tombstones, protected runtime invariants and historical compatibility.
...
- T-010 [x] Run exact integrity/scope checks; exact GitHub compare is within Issue #229 Scope, Change Contract validation reports CONTROL_PLANE with the exact changed-file set, and repository validation reports no forbidden tracked runtime/secret artifacts.
- T-011 [x] Open PR #230 with exact Change Contract, Risk profile REQUIRED, Quality verdict CONCERNS with the bounded post-merge activation finding, and full deployment transaction audit linkage.
...
- AC-007 | Task: T-006,T-008 | Evidence: legacy request workflows/parsers absent; canonical-inventory verifier/policy paths are fail-closed non-authoritative tombstones | Coverage: COVERED
...
- AC-010 | Task: T-010,T-011 | Evidence: exact GitHub compare scope, Change Contract validation and repository artifact/secret policy validation | Coverage: COVERED
...
- [x] Exact changed-file scope verified — exact branch diff is within Issue #229 approved paths and matches PR #230 Change Contract.
