...
- FR-004: When valid local HLS is static, the helper MUST decode one frame from exactly `rtsp://10.123.239.102:8554/cam1` over TCP before any restart.
...
- AC-002: Unit test proves static HLS plus a successful fixed relay frame probe restarts exactly `sea-speed-camera1-h264.service` and then requires advancing HLS.
...
- NFR-004 | Area: BACKWARD_COMPATIBILITY | Target: preserve Auth v1 reconcile, browser URL and existing camera/relay topology | Validation: existing privilege-boundary/Auth tests plus unchanged routing source | Evidence: only privileged helper, focused test, runbook and linked SDD change; camera/nginx/worker configs remain absent from diff | Status: PASS
- NFR-005 | Area: OPERABILITY | Target: stale valid HLS self-recovers through canonical deployment while healthy HLS is no-op | Validation: no-op/restart test scenarios and production HLS sequence evidence | Evidence: source tests PASS; production evidence pending separate authorization | Status: CONCERNS
...
- This narrows the observed fault to the VPS H264 compatibility producer path rather than the physical camera, Ubuntu relay, browser cache, Water worker, nginx, or Authentik.
...
- Correct-course design therefore keeps `deploy/vps/deploy.sh` behavior unchanged and extends the exact-source-bound privileged reconcile transaction with a fixed Camera 1 freshness verification/recovery stage. This minimizes changed runtime surface while still making stale Camera 1 HLS a deployment acceptance gate.
- Production mutation is not authorized by source approval. After exact-green merge and exact-main Quality, the release requires separate exact-SHA production authorization and one exact-source privilege-boundary bootstrap before canonical VPS deployment.
