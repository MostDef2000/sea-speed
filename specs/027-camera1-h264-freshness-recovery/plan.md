...
- D-005: Before restarting H264, require one decodable frame over TCP from exactly `rtsp://10.123.239.102:8554/cam1`; the fixed production VPS FFmpeg probe uses `-rtsp_transport tcp -timeout 10000000` and never the unsupported `-rw_timeout` option.
...
Source validation uses focused Python unit tests for the root helper plus existing Auth/installer tests. The test harness captures every privileged command and proves the fixed constants, exact VPS-compatible FFmpeg relay-probe argv, absence of `-rw_timeout`, no-op behavior, relay-before-restart ordering, one exact H264 restart, failure-before-restart when relay media is absent, and failure when post-restart HLS remains static. A deployment-contract assertion proves both accepted `runtime_verified` paths remain behind `run_auth_boundary`, which now includes Camera 1 freshness recovery.

The corrective PR diff must remain a subset of the five freshly authorized Issue #226 production-learning paths. PR Validation and aggregate Quality must pass on one exact head. Before merge, refresh current `main`, exact PR head, changed-file scope, review submissions and unresolved threads; merge only with expected-head protection. Exact-main Quality must pass before any new production decision.

Production acceptance is separate: compute a new fingerprint for the corrective merge, obtain fresh exact-SHA production authorization, install the new exact-source helper bundle once through the repository-owned root bootstrap, then use canonical GitHub Actions VPS deployment for that exact authorized release. Acceptance requires HLS freshness PASS in deployment evidence and an authenticated browser showing current advancing video.
...
- TEST-002 | Covers: AC-002,AC-006 | Level: unit | Priority: P0 | Evidence: static HLS + fixed relay frame uses exact `ffmpeg ... -rtsp_transport tcp -timeout 10000000 ...`, rejects `-rw_timeout`, causes exactly one `systemctl restart sea-speed-camera1-h264.service`, and contains no forbidden service names
...
- TEST-008 | Covers: AC-009,AC-010 | Level: integration | Priority: P0 | Evidence: exact authorized-subset corrective PR diff, exact-head PR Validation + aggregate Quality, expected-head merge, exact-main Quality
- TEST-009 | Covers: AC-011,AC-012 | Level: end-to-end | Priority: P0 | Evidence: fresh exact-SHA authorization + exact-source privilege bootstrap PASS followed by canonical VPS deployment `runtime_verified`
...
- Issue impact: Issue #226 remains the same stale clean-stream Outcome; the correction removes a production-only FFmpeg CLI incompatibility discovered by its first authorized deployment attempt.
- Specification impact: binds the fixed relay probe to the production-supported `-timeout 10000000` option and explicitly rejects `-rw_timeout`.
- Plan impact: preserves the existing reconcile/restart architecture and adds deterministic CLI-compatibility coverage before retry.
- Tasks impact: records the first merge/authorization/bootstrap as completed, the failed deployment/rollback learning, and a corrective source/CI/merge cycle before a fresh production authorization.
- Authorization impact: the previous production authorization remains historical evidence for `28e9f4f...` only; the corrective merge requires a new fingerprint and exact-SHA production authorization.
- Follow-up: integrate the corrective exact-green source, compute its fingerprint, obtain fresh production authorization, perform one new exact-source helper bootstrap, run canonical VPS deployment, verify advancing HLS/browser output, then return to Issue #218 natural-vessel Water Passage acceptance.

## Deployment transaction audit

- Adjacent-stage review: COMPLETE
- Production-learning root cause: the first authorized deployment of `28e9f4f30554de40ace25ddc1e65184e9a68e6b8` passed exact-source privilege-boundary status and protected HTTP preflight, then reached the Camera 1 freshness PRE-MUTATION stage. Local HLS was static, so the helper invoked its fixed private-relay frame probe. The installed production VPS FFmpeg rejected `-rw_timeout` before opening the RTSP input (`Option rw_timeout not found`), which the helper correctly treated as relay-probe failure. No Camera 1 service restart occurred, candidate acceptance failed, and the canonical deployment rolled source files back successfully to the previous accepted VPS release.
- Production-learning adjacent-stage findings: ADMISSION and exact-source privilege bootstrap were valid; PRE-MUTATION fail-closed behavior worked and prevented an unsafe restart; MUTATION was never entered; rollback restored the accepted source; a bounded read-only operator probe using `-rtsp_transport tcp -timeout 10000000` opened the same fixed relay and reached HEVC decoding, isolating the immediate defect to helper argv compatibility rather than relay topology. The correction therefore changes one fixed option token plus deterministic regression coverage, with no topology or deployment-script change.

- TX-001 | Stage: ADMISSION | Mutation: NO | Failure disposition: FATAL | State after failure: previous accepted VPS runtime remains active; candidate cannot proceed | Retry: only exact main first-parent corrective release with green exact-main Quality and fresh durable production authorization | Rollback: not applicable before mutation | Evidence: source SHA, Issue #226, corrective merged PR, Quality status and production authorization verifier
- TX-002 | Stage: PRE-MUTATION | Mutation: NO | Failure disposition: FATAL | State after failure: no Camera 1 service restart occurs | Retry: only after exact-source helper bundle is installed and local HLS/private relay prerequisites are re-evaluated with the fixed VPS-compatible `-timeout 10000000` probe | Rollback: not applicable because helper validates exact bundle and probes before restart | Evidence: privilege-boundary status PASS, valid local HLS static evidence, exact fixed FFmpeg argv, fixed private relay frame probe
...
- The first authorized #226 VPS deployment for `28e9f4f30554de40ace25ddc1e65184e9a68e6b8` passed exact-source privilege and protected-route preflight, then failed at the private-relay frame probe because production FFmpeg reported `Option rw_timeout not found`; no H264 restart was allowed and rollback to `0af31b5e2516fb0d529228a51025693e7a932779` succeeded.
- The bounded read-only follow-up probe against `rtsp://10.123.239.102:8554/cam1` with `-timeout 10000000` reached HEVC decoding, confirming the immediate deployment failure was the unsupported FFmpeg option rather than evidence that the relay route itself was unreachable.
...
- The corrective source preserves `deploy/vps/deploy.sh` bytes and its existing reconcile-before-`runtime_verified` invariant; Camera freshness failure continues to propagate through the same privileged reconcile result.
- Production remains pending a new exact-SHA authorization for the corrective merge. Because helper bytes change again, that exact release will require one new repository-owned privilege-boundary bootstrap before canonical VPS deployment.