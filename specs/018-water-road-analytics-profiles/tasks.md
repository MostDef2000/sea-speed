...
- TASK-001: Keep repository writes within the exact authorized 42 paths recorded on Issue #197; no physical road camera source, credentials, model binary, Authentik/MediaMTX changes or Windows-specific Road feature additions.
- TASK-002: Add `worker/analytics_profiles.py` with `water-v1`/`road-v1` defaults and deterministic class normalization.
- TASK-003: Route in-process and supervised Ubuntu YOLO detections through the profile registry and add semantic fields to events/state while preserving tracking/speed behavior and legacy no-profile compatibility used by shared worker consumers.
...
- TASK-011: Open one PR linked to Issue #197, maintain exact 42/42 changed-file scope and remediate only in-scope CI defects until PR Validation, Quality integration and Package Windows Worker are green on one exact final head.
...
- TASK-013: After source merge, do not mutate production until a fresh exact merged-SHA production envelope is authorized; then run the documented VPS/Ubuntu/model/dual-worker/Windows/browser acceptance transaction.
- TASK-014: Treat Windows as compatibility/update only: use the existing Windows package/runtime contract for the changed shared `worker/**` source; add no Windows-specific Road service/UI and require exact package/source/process/freshness/applicable telemetry evidence after separate production authorization.
...
- AC-003 | Task: TASK-003,TASK-010,TASK-014 | Evidence: worker tracking/AI supervision contract tests and final-head Windows packaging | Coverage: COVERED
...
- [x] Issue/spec/plan/tasks current: Issue #197 and feature 018 record the product outcome, risks, test design and corrected VPS + Ubuntu + Windows runtime applicability.
- [ ] Exact changed-file scope verified: final branch compare equals the authorized 42 paths and nothing else.
- [ ] Required tests and evidence complete: profile, Worker, API/data, frontend, Ubuntu exact-release, VPS deploy, telemetry, artifact and Windows package compatibility checks pass.
- [ ] Required CI green: PR Validation, aggregate Quality integration and Package Windows Worker succeed on the same exact final head.
- [ ] Exact-green-head merge complete: fresh main/head/scope/review gate plus expected-head merge.
- [ ] Deployment state resolved: source merge alone leaves production pending; later exact-SHA production transaction must resolve VPS + Ubuntu + Windows when authorized.
- [ ] Runtime acceptance resolved: later evidence must prove exact `yolo26x.pt` digest/CUDA load, both Ubuntu worker progression/resource headroom, protected road1 binding, exact Windows package/source/process/freshness/applicable telemetry and browser smoke.
- [x] Deferred work recorded: custom maritime model training/broader vessel taxonomy remain outside this Outcome; no Windows-specific Road feature is planned here.
- [ ] Risks resolved or explicitly accepted: RISK-002/RISK-005/RISK-007 remain open until production dual-worker/model/Windows evidence; RISK-006 is explicitly accepted as baseline taxonomy limitation.
- [x] Waivers resolved or current: no source-quality waiver is requested.

## Completion gate

Source integration is ready for the production decision only after exact 42-path merge and successful post-merge quality. The overall Outcome becomes `DONE` only after a separately authorized production transaction resolves every canonical runtime contour: VPS, Ubuntu Worker and Windows Worker. Required evidence includes the exact model/CUDA gate, both Ubuntu analytics workers, protected road camera binding, resource headroom, exact Windows package/source/process/freshness/applicable telemetry and authenticated browser acceptance. Until then production is intentionally pending rather than implicitly authorized.
