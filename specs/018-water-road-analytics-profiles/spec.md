...
- Second corrective source authorization: a fresh six-field 12-path VPS-only Scope was shown and immediately approved with `OUTCOME APPROVED` on 2026-08-17. Its source goal is to close the deployment-automation gap without changing Ubuntu, Windows, API/frontend product semantics, topology or worker source.
- Second corrective VPS production: REQUIRED only after the new 12-path corrective source is merged and receives a fresh exact-SHA production authorization. The protected deployment must emit a passed `auth_v1_road_private_m2m` manifest check before VPS acceptance.
- Pending Ubuntu Worker/relay production from the first correction remains paused until the second corrective VPS boundary is green. This second source diff itself has Ubuntu Worker/relay NOT APPLICABLE and Windows Worker NOT APPLICABLE.
