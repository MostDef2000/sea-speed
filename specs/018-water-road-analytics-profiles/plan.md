...
- TX-002 | Stage: PRE-MUTATION | Mutation: POSSIBLE | Failure disposition: FATAL | State after failure: live API/frontend/nginx/current-release/deployment-manifest remain unchanged; only exact candidate release-directory staging may have occurred | Retry: install/reinstall exact privilege bundle with one authorized root bootstrap, then rerun Connector | Rollback: staged release may remain; no live rollback required | Evidence: exact artifact includes helper/installer/cutover/renderers; helper `status` exact SHA/digest markers; `PRIVILEGE_BOUNDARY_BOOTSTRAP_REQUIRED=YES` on absence/mismatch
