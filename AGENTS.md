...
23. In a synchronous session, when no safe action is executable now and progress depends only on a named machine-observable external transition other than a known GitHub Actions run/check that is `queued` or `in_progress`, persist `WAITING_EXTERNAL` and return control without promising background work. A known GitHub Actions run/check that is `queued` or `in_progress` is not a `WAITING_EXTERNAL` condition; the invocation remains `ACTIVE`, foreground-waits, and re-observes only the exact run/check cursor through the official Connector at least 30 seconds after the previous equivalent status observation, without an observation count or deadline that itself hands control back, without busy/tight polling, and without checkpoint-generation churn per observation.
24. A resumed non-CI `WAITING_EXTERNAL` checkpoint permits one bounded observation of its exact evidence cursor. Unchanged non-CI evidence returns `WAITING_EXTERNAL` without replanning, polling, or incrementing checkpoint generation; changed evidence resumes automatic execution. A persisted pre-amendment CI wait follows the compatibility upgrade rule below.
...
28. Every startup status block and every user-visible wait, blocker, decision or terminal result includes a concise todo summary: current item, items completed since the prior visible transition, remaining/waiting items, `Model / orchestrator` and `Model / active worker` directly under the todo lines.
...
A persisted pre-amendment `WAITING_EXTERNAL` checkpoint whose exact cursor is a known GitHub Actions run/check is upgraded to `ACTIVE` after its one Resume Probe observation even when the run remains queued/in-progress; the same invocation then applies foreground CI observation without repeating source authorization.

For non-CI external waits, Sea Speed sessions are synchronous. After writing `WAITING_EXTERNAL`, the Orchestrator returns control and performs no background polling. A later invocation runs one bounded Resume Probe. If the referenced cursor is unchanged, it returns the same wait without broad recovery, repeated planning or a generation update. If evidence changed, it clears the wait and executes the next admissible action. If another safe action is executable now, `ACTIVE` takes precedence.
...
Explicit forbidden implicit fallbacks include Gmail, Calendar, Drive, Notion, `gh`, local GitHub authentication, `git push`, manual GitHub web publication, ad-hoc SSH/shell exploration, direct database mutation, cloud-console mutation and every other unlisted connector/plugin/service.
...
`FAILED` is not a terminal interaction state; it is an internal event. `WAITING_EXTERNAL` is not terminal and does not promise background work. PR created, merge ready, release built, deployment started, or checkpoint updated are not terminal while a safe authorized next action is executable now. A known GitHub Actions run/check that is `queued` or `in_progress` remains `ACTIVE` with foreground rate-limited exact-cursor observation and is not a reason to return `WAITING_EXTERNAL`. Non-CI external state uses `WAITING_EXTERNAL` instead of polling or replanning.
