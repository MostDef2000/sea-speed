...
For deployment, the default operator round-trip budget is **one copy-paste action and one returned sanitized result per deployment contour or independently authorized stage whenever technically safe**. This is an orchestration requirement, not a presentation preference. The PM MUST maximize the safe bounded step before exposing commands to the operator and MUST NOT turn deterministic internal phases into separate operator prompts merely because they are implemented as separate scripts, checks or commands.
...
- Do not ask the operator to run separate routine steps for preflight, known dependency preparation, exact-SHA/integrity verification, install/update, activation/restart, bounded health/runtime checks, evidence collection, safe retry or rollback when those phases can be encoded safely in the repository-owned entrypoint for the current authorized stage.
- A known deterministic dependency prerequisite that repeatedly requires `prepare -> manual install -> rerun` is a deployment-UX defect unless a protected provider/credential boundary genuinely prevents repository-owned preparation. Fix the deployment entrypoint through the source lifecycle instead of normalizing repeated operator input.
...
- If a failure is already understood and its remediation is deterministic, reversible and within the existing authorization envelope, the normal next operator step SHOULD remediate, retry the failed stage and collect acceptance evidence in the same round trip rather than asking for separate repair and retry commands.
- Treat avoidable sequences such as `preflight -> dependency install -> prepare again -> activate -> health check -> journal fetch` as deployment-UX defects when a repository-owned bounded entrypoint can safely own the sequence.
...
The target is **one operator copy-paste and one sanitized result per deployment contour or independently authorized stage** whenever the complete stage can be made deterministic, bounded and fail-closed. Any additional round trip needs a concrete safety, protected-interaction or independent-failure-domain reason.
...
- Put deterministic integrity verification, exact-source binding, archive extraction, prerequisite discovery, known dependency preparation, known-path resolution, target checks, backups/rollback preparation, bounded health/smoke validation, evidence collection and safe retry logic inside the reviewed repository entrypoint when applicable.
...
- Do not require the operator to manually download/unpack a bundle, compare hashes, export redundant environment variables, repeat the same non-secret input, perform a known dependency bootstrap as a separate routine step, or run separate diagnostic probes when the target-local repository entrypoint can do the same work deterministically.
- If a repository entrypoint fails, prefer one bounded repo-owned diagnostic/remediation operation that collects sanitized evidence, fixes the known issue when safe and idempotent, retries the failed stage when doing so does not cross a new protected boundary, and emits the final acceptance state in the same operator round trip.
