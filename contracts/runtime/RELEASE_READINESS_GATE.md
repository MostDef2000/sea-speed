...
`WAITING_EXTERNAL` is a nonterminal synchronous-session disposition, not a lifecycle phase or release verdict. It is valid only when no safe authorized action is executable now and one named machine-observable external transition other than a known GitHub Actions run/check that is `queued` or `in_progress` is the sole prerequisite. Delivery Checkpoint v2 records the condition, resume trigger, exact evidence cursor and non-executable next action. A known GitHub Actions run/check that is `queued` or `in_progress` remains `ACTIVE`, foreground-waits on only the exact run/check cursor through the official Connector, and is observed no sooner than at least 30 seconds after the previous equivalent status read. No observation count or deadline may itself hand control back; no busy/tight polling or checkpoint-generation churn occurs per observation. Success continues to the next gate; failure immediately narrows to failed job/log remediation. A Connector/provider capability outage is `BLOCKED` or `HUMAN DECISION REQUIRED`, not CI pending. A later Resume Probe observes a non-CI wait cursor once: unchanged evidence preserves the wait and generation; changed evidence produces valid `ACTIVE` state. No background polling is implied.

A persisted pre-amendment CI wait remains readable: its one exact Resume Probe observation upgrades it to `ACTIVE` even if the run is still queued/in-progress, after which the same invocation applies foreground observation.
