...
for command_name in docker curl hostname sha256sum sed tail; do
...
curl --fail --silent --show-error --max-time 5 \
  ...
  || fail "AUTHENTIK_LOOPBACK_HEALTH" 50
...
')" || fail "PROVIDER_QUERY" 51
  flow="$(printf '%s\n' "$output" | sed -n 's/^SEA_SPEED_PROVIDER_FLOW=//p' | tail -n 1)"
  [[ -n "$flow" ]] || fail "PROVIDER_FLOW_EMPTY" 52
  printf '%s' "$flow"
}

copy_and_apply_blueprint() {
  local source_path="$1" container_path
  container_path="/tmp/$(basename "$source_path")"
  docker cp "$source_path" "${worker_id}:${container_path}" >/dev/null \
    || return 1
  if ! docker exec "$worker_id" ak apply_blueprint "$container_path"; then
    docker exec "$worker_id" rm -f "$container_path" >/dev/null 2>&1 || true
    return 1
  fi
  docker exec "$worker_id" rm -f "$container_path" >/dev/null 2>&1 || true
}
...
case "$mode:$current_flow" in
  ...
  *) fail "UNEXPECTED_CURRENT_FLOW_${current_flow}" 53 ;;
esac
...
if ! copy_and_apply_blueprint "$selected_source"; then
  fail "APPLY_BLUEPRINT_COMMAND" 54
fi
  ...
  if [[ "$mode" == "apply" && "$current_flow" == "$rollback_target" ]]; then
    if copy_and_apply_blueprint "$rollback_source"; then
      ...
      if [[ "$restored_flow" == "$rollback_target" ]]; then
        ...
        fail "PROVIDER_VERIFY_ROLLED_BACK" 55
      fi
    fi
    printf 'AUTHENTIK_LOGOUT_AUTO_ROLLBACK=FAILED\n' >&2
  fi
  fail "PROVIDER_VERIFY_${after_flow}" 55
fi
...
[[ "$server_after" == "$server_id" ]] || fail "SERVER_CONTAINER_DRIFT" 56
[[ "$worker_after" == "$worker_id" ]] || fail "WORKER_CONTAINER_DRIFT" 56
[[ "$postgres_after" == "$postgres_id" ]] || fail "POSTGRES_CONTAINER_DRIFT" 56

curl --fail --silent --show-error --max-time 5 \
  http://127.0.0.1:9000/-/health/ready/ >/dev/null \
  || fail "AUTHENTIK_LOOPBACK_HEALTH_AFTER" 57
