...
2. creates a temporary clean Git repository under `shared/runtime`;
3. fetches `origin/main` from the canonical repository;
4. proves that the requested commit is an ancestor of `origin/main`;
5. checks out that exact commit in detached mode;
6. verifies `quality-integration=success` for the exact commit;
7. invokes `install-manual.sh` for the immutable release;
8. preserves shared config, models, datasets and output;
9. exits without changing or restarting the active systemd service.
...
```text
/opt/sea-speed-worker/shared/runtime/update.lock
```

A second updater process fails instead of modifying release or service state concurrently.
